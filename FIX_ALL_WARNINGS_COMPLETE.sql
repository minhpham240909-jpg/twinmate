...
-- =====================================================
-- STEP 2: Fix AI Agent Tables (agent_memory, agent_task, etc)
-- =====================================================
...
USING (true);  -- All users can view for partner matching
...
-- 2d. MATCH_CANDIDATE (uses PascalCase table name and userId field)
DROP POLICY IF EXISTS "Users can view own match candidates" ON "MatchCandidate";
DROP POLICY IF EXISTS "Users can insert own match candidates" ON "MatchCandidate";

CREATE POLICY "Users can view own match candidates"
ON "MatchCandidate" FOR SELECT TO authenticated
USING ((select auth.uid())::text = "userId"::text);

CREATE POLICY "Users can insert own match candidates"
ON "MatchCandidate" FOR INSERT TO authenticated
WITH CHECK ((select auth.uid())::text = "userId"::text);

-- =====================================================
-- STEP 3: Create/Fix LearningProfile Table
-- =====================================================
...
BEGIN
  ...
  RAISE NOTICE '  ✓ agent_memory - 4 policies optimized';
  RAISE NOTICE '  ✓ agent_task - 3 policies optimized';
  RAISE NOTICE '  ✓ availability_block - 4 policies optimized';
  RAISE NOTICE '  ✓ match_candidate - 2 policies optimized';
  RAISE NOTICE '  ✓ LearningProfile - 4 policies optimized';
