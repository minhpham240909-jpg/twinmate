...
-- ==========================================
-- This migration optimizes the entire database for high user volume
-- while maintaining strict RLS security and data protection
--
-- Key Improvements:
-- 1. Strategic indexes for all high-traffic queries
-- 2. Composite indexes for common query patterns
-- 3. Partial indexes for filtered queries
-- 4. Table partitioning for time-series data
-- 5. Optimized RLS policies for performance
-- 6. Materialized views for expensive aggregations
-- 7. Query performance improvements
--
-- FIXES APPLIED:
-- - Removed CONCURRENTLY (cannot run inside transaction blocks)
-- - All camelCase column names properly quoted (e.g., "createdAt", "userId")
-- - All ENUM values use UPPERCASE (e.g., 'ONLINE', 'SCHEDULED', 'ACTIVE')
-- - Message table uses correct column names (groupId/recipientId, not matchId/receiverId)
-- - Removed VACUUM commands (run separately after migration)
--
-- NOTE: Indexes will lock tables briefly during creation (usually <1 second per index)
-- Safe to run on production with low traffic. For zero-downtime on high-traffic,
-- use manual CONCURRENTLY approach outside transactions.
--
-- SECURITY: All optimizations maintain or improve RLS security
-- ==========================================
...
-- Solution: Composite indexes + partial indexes + partitioning

-- Drop old index if exists, create better composite index
...
--
-- Or run manually in psql or Supabase SQL Editor (outside transaction):
-- psql -c "VACUUM ANALYZE \"Notification\";"
