...
export default function DashboardPage() {
  ...
  // Online partners state - initialize from cache (with activity type)
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          // Filter and set online partners with activity type
  ...
  // FIX: Refresh online partners when user returns to page (tab visibility change)
  // This ensures online status is updated when returning from AI sessions, study calls, etc.
  useEffect(() => {
    if (!user) return

    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        // User returned to page - refresh online partners immediately
        refreshOnlinePartners()
      }
    }
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <header className="bg-neutral-50 dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-800 px-8 py-6 sticky top-0 z-10">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {/* Chat Messages Notification - Quick access to chat with unread indicator */}
              <button
                onClick={() => router.push('/chat')}
              ...
              >
                ...
                {unreadMessagesCount > 0 && (
                  <span className="absolute top-1 right-1 min-w-5 h-5 px-1 bg-gradient-to-r from-blue-500 to-blue-600 text-white text-xs font-bold rounded-full flex items-center justify-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-gradient-to-br from-orange-500 to-amber-500 rounded-2xl p-5 text-white shadow-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium opacity-90">{t('studyStreak') || 'Study Streak'}</span>
              </div>
              <p className="text-4xl font-black">{userStats?.streak.current || 0}</p>
            ...
            <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-5 text-white shadow-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium opacity-90">{t('todayStudy') || 'Today'}</span>
              </div>
              <p className="text-4xl font-black">{userStats?.studyTime.today.display || '0m'}</p>
            ...
            <div className="bg-gradient-to-br from-emerald-500 to-green-500 rounded-2xl p-5 text-white shadow-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium opacity-90">{t('thisWeek') || 'This Week'}</span>
              </div>
              <p className="text-4xl font-black">{userStats?.studyTime.thisWeek.display || '0m'}</p>
            ...
            <div className="bg-gradient-to-br from-purple-500 to-violet-600 rounded-2xl p-5 text-white shadow-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium opacity-90">{t('totalPoints') || 'Points'}</span>
              </div>
              <p className="text-4xl font-black">{userStats?.points || 0}</p>
