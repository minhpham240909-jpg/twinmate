...
export default function CreatePostPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      {/* Header */}
      <header className="bg-gray-50 dark:bg-slate-900/40 backdrop-blur-xl border-b border-gray-200 dark:border-slate-700/50 sticky top-0 z-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <button
              onClick={() => router.push('/community')}
              className="flex items-center gap-2 text-gray-600 dark:text-slate-400 hover:text-gray-900 dark:hover:text-slate-100 transition-colors group"
            >
        ...
        <FadeIn delay={0.1}>
          <GlowBorder color="#8b5cf6" intensity="medium" animated={false}  style={{ borderRadius: 16 }}>
            <motion.div
              ...
              className="bg-white dark:bg-slate-900/40 backdrop-blur-xl rounded-2xl shadow-xl border border-gray-200 dark:border-slate-700/50 overflow-hidden"
            >
          {/* User Info */}
          <div className="px-6 py-4 border-b border-gray-200 dark:border-slate-700/50 bg-gradient-to-r from-gray-100 dark:from-slate-800/50 to-gray-50 dark:to-slate-800/30">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-900 dark:text-slate-100">{profile?.name || 'User'}</p>
                <p className="text-sm text-gray-600 dark:text-slate-400">{t('postingToCommunity')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
              ...
              {activeTab === 'text' && (
                ...
                >
                  <textarea
                    ...
                    className="w-full p-4 bg-white dark:bg-slate-800/50 border-2 border-gray-200 dark:border-slate-600/50 rounded-xl resize-none focus:outline-none focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/20 transition-all text-gray-900 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-500 min-h-[200px] text-lg backdrop-blur-sm"
                    rows={8}
                    maxLength={5000}
                  />
                  ...
                  {showMentions && mentionUsers.length > 0 && (
                    <div className="absolute z-10 mt-2 w-full bg-white dark:bg-slate-800/95 backdrop-blur-xl border border-gray-200 dark:border-slate-600/50 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                      {mentionUsers.map((user) => (
                        <button
                          key={user.id}
                          onClick={() => insertMention(user)}
                          className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-100 dark:hover:bg-slate-700/50 transition text-left"
                        >
                          {user.avatarUrl ? (
                            <img
                              src={user.avatarUrl}
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 bg-white dark:bg-slate-800/50 border-2 border-gray-200 dark:border-slate-600/50 rounded-xl focus:outline-none focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/20 transition-all text-gray-900 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-500 backdrop-blur-sm"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                      ...
                      <textarea
                        ...
                        className="w-full p-4 bg-white dark:bg-slate-800/50 border-2 border-gray-200 dark:border-slate-600/50 rounded-xl resize-none focus:outline-none focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/20 transition-all text-gray-900 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-500 min-h-[120px] backdrop-blur-sm"
                        rows={4}
                        maxLength={5000}
                      />
                    </div>
                ...
                >
                  <div className="space-y-4">
                    ...
                    {imagePreviewUrls.length === 0 ? (
                      <label className="block w-full border-2 border-dashed border-gray-200 dark:border-slate-600/50 rounded-xl p-12 text-center hover:border-blue-500/50 hover:bg-gray-50 dark:hover:bg-slate-800/30 transition-all cursor-pointer group">
                        <input
                          type="file"
                          accept="image/*"
                          multiple
                        ...
                        <div className="flex flex-col items-center gap-4">
                          ...
                          <div>
                            <p className="text-lg font-medium text-gray-900 dark:text-slate-200">Click to upload images</p>
                            <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">Up to 4 images (PNG, JPG, GIF)</p>
                          </div>
                        </div>
                      </label>
                    ) : (
                      <div className="grid grid-cols-2 gap-4">
                        {imagePreviewUrls.map((url, index) => (
                          <div key={index} className="relative group">
                            <img
                              src={url}
                              alt={`Preview ${index + 1}`}
                              className="w-full h-48 object-cover rounded-xl border-2 border-gray-200 dark:border-slate-600/50"
                            />
                            <button
                        ...
                        {imagePreviewUrls.length < 4 && (
                          <label className="border-2 border-dashed border-gray-200 dark:border-slate-600/50 rounded-xl p-8 text-center hover:border-blue-500/50 hover:bg-gray-50 dark:hover:bg-slate-800/30 transition-all cursor-pointer flex items-center justify-center">
                            <input
                              type="file"
                              accept="image/*"
                              multiple
                            ...
                            <div className="flex flex-col items-center gap-2">
                              ...
                              <span className="text-sm text-gray-600 dark:text-slate-400">{t('addMore')}</span>
                            </div>
                          </label>
                        )}
                      </div>
            ...
            <div className="mt-6 pt-6 border-t border-gray-200 dark:border-slate-700/50">
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                />
                <div>
                  <span className="font-medium text-gray-900 dark:text-slate-100">{t('allowSharing')}</span>
                  <p className="text-sm text-gray-600 dark:text-slate-400">{t('allowSharingDescription')}</p>
                </div>
              </label>
            </div>
          </div>
          ...
          {/* Footer Actions */}
          <div className="px-6 py-4 bg-gray-100 dark:bg-slate-800/30 backdrop-blur-sm border-t border-gray-200 dark:border-slate-700/50 flex items-center justify-between">
            <button
              onClick={() => router.push('/community')}
              className="px-6 py-2.5 text-gray-700 dark:text-slate-300 font-medium rounded-xl hover:bg-gray-200 dark:hover:bg-slate-700/50 transition-colors"
            >
              Cancel
            </button>
            <Bounce>
